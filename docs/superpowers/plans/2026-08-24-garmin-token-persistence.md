...
Cover valid JSON; missing, null, empty, and non-string required fields; malformed JSON; and equivalent JSON with different key order/whitespace.
...
Cover old positional secret compatibility, `--tokenstore`, mutual exclusion, neither supplied, normal final checkpoint, and business failure after a simulated refresh.
...
Use an argparse mutually exclusive group for positional `secret_string` and `--tokenstore`. Construct via `from_tokenstore` when supplied. Wrap all business synchronization after client construction in `try/finally`; call `persist_tokenstore()` in `finally` only for file mode. Preserve exception context if both business logic and persistence fail.
...
- PUT 401 raises a distinct `GitHubStateAuthError` so the workflow can reissue an App token once.
...
Provide `fetch` and `put` subcommands. Read authentication only from `GH_TOKEN`; write ciphertext and metadata to explicit files under `$RUNNER_TEMP`; emit only safe metadata to stdout/GitHub outputs. Use an overridable API base URL for tests.
...
- state persistence runs with `always()`, handles only auth retry, and final data push uses a separately generated App token.
- dry-run skips only the generated-data commit/push.
- generated-data staging uses the approved allowlist and never `git add .`.
...
- [ ] **Step 3: Add SOPS configuration after the age recipient is available**
...
- [ ] **Step 4: Rewrite the workflow in small, named steps**
...
Install SOPS 3.13.3 from its official release and verify the exact checksum before `chmod`/installation. Fetch encrypted state with the read-only job token, decrypt or bootstrap in `$RUNNER_TEMP`, record semantic digest, run sync with `--tokenstore`, make a local generated-data commit on success, then perform the `always()` SOPS round-trip and Contents CAS.

If the first CAS returns only `GitHubStateAuthError`, issue one new App token and retry with the same blob SHA. A conflict or any other error must fail immediately. Reissue another App token just before rebasing/pushing generated data.

- [ ] **Step 5: Add plaintext defenses**
...
- [ ] **Step 6: Run static workflow tests and YAML validation**
...
- [ ] **Step 3: Pause for the user to configure external state**

Do not create the GitHub App, ruleset bypass, Environment, or Secrets without explicit user confirmation. Obtain the age public recipient before finalizing `.sops.yaml`. Never request that the private age key or App private key be pasted into chat.
