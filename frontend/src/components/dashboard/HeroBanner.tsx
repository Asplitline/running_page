...
import { overallStats, statsByYear, longestStreak, latestMonthKm, ANNUAL_GOAL_KM } from '@/lib/stats';
...
const ProgressRing = ({ pct, center, year }: { pct: number; center: string; year: number }) => {
  const r = 38;
  ...
  return (
    <div className="relative h-[92px] w-[92px] shrink-0">
      <svg width="92" height="92" viewBox="0 0 92 92">
        <circle cx="46" cy="46" r={r} fill="none" stroke="var(--color-bar-muted)" strokeWidth="7" />
        <circle
          cx="46"
          cy="46"
          r={r}
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="tnum text-lg font-extrabold text-[var(--color-ink)]">{center}</span>
        <span className="font-mono text-[8px] tracking-wide text-[var(--color-ink-3)]">{year}</span>
      </div>
    </div>
  );
};
...
const Cell = ({
  ...
  const inner = (
    <>
      <div className="tnum text-lg font-extrabold tracking-tight text-[var(--color-ink)]">
        {value}
        {unit && (
          <span className="text-[11px] font-semibold text-[var(--color-ink-3)]"> {unit}</span>
        )}
      </div>
      <div className="mt-1 font-mono text-[9px] tracking-wide text-[var(--color-ink-3)] uppercase">
        {label}
      </div>
    </>
  );
  ...
  return to ? (
    <Link to={to} className={`${cls} block transition-colors hover:text-[var(--color-accent)]`}>
      {inner}
    </Link>
  ) : (
...
const YearRow = ({
  ...
  <div>
    ...
    <div
      className="tnum text-[clamp(30px,4.4vw,44px)] leading-none font-extrabold tracking-tight text-[var(--color-ink)]"
      style={{ fontFamily: 'var(--font-display)' }}
    >
      {km}
      <span className="text-[13px] font-semibold text-[var(--color-ink-3)]"> km</span>
    ...
    <div className="mt-8 grid overflow-hidden rounded-[var(--radius-card)] border border-[var(--color-line)] bg-[var(--color-card)] shadow-[var(--shadow-soft)] lg:grid-cols-[1.05fr_0.95fr]">
      ...
      <div className="flex flex-col justify-between gap-9 p-9 sm:p-11">
        <div>
          ...
          <div className="mt-3 mb-2 flex items-end gap-3 leading-[0.85]">
            <span
            ...
            <span className="mb-3 text-lg font-semibold text-[var(--color-ink-3)]">km</span>
          </div>
          <p className="text-[13px] text-[var(--color-ink-2)]">
            {s.totalRuns} 次跑步 · 累计坚持 · 最长单次{' '}
            <b className="font-bold text-[var(--color-ink)]">{formatKm(s.longestRunKm)}km</b>
          </p>
        </div>

        {/* 年度目标进度环 */}
        <div className="flex flex-wrap items-center gap-7">
          <ProgressRing pct={goalPct} center={`${Math.round(goalPct * 100)}%`} year={year} />
          <div>
            <div className="text-[13px] text-[var(--color-ink-2)]">
              <b className="tnum text-lg font-bold text-[var(--color-ink)]">{formatKm(s.thisYearKm)}</b>{' '}
              / {ANNUAL_GOAL_KM} km
            </div>
        ...
        <div className="grid grid-cols-4 border-t border-[var(--color-line)]">
          <Cell value={formatKm(s.thisYearKm)} unit="km" label={`${year} 至今`} />
          <Cell value={formatKm(month.km)} unit="km" label={`本月 ${month.month.slice(5)}`} />
      ...
      {/* 右半：三年逐年对比 */}
      <div className="flex flex-col border-t border-[var(--color-line)] bg-[var(--color-card-2)] p-9 sm:p-11 lg:border-t-0 lg:border-l">
        <div className="mb-5 flex items-baseline justify-between">
          <span className="font-mono text-[11px] tracking-widest text-[var(--color-ink-3)] uppercase">
            逐年对比
          </span>
          <span className="font-mono text-[9px] text-[var(--color-ink-3)]">{year} 为年中进度</span>
        </div>
        <div className="flex flex-1 flex-col justify-between gap-6">
          {years.map((y, i) => {
            const prev = i > 0 ? years[i - 1] : null;
            const deltaKm = prev && prev.km > 0 ? Math.round(((y.km - prev.km) / prev.km) * 100) : null;
            const fasterSec = prev ? Math.round(prev.avgPaceSec - y.avgPaceSec) : null;
